    ...
    <div className="bg-[#f2e6c8] min-h-screen">
      ...
      <section className="bg-[#1a3a72] border-b-4 border-[#cc3322] pt-16 pb-0 px-8 md:px-16 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto">
          ...
          <div className="pb-16">
            <p className="font-ui text-[11px] uppercase tracking-[0.28em] text-[#4A6A9B] mb-5">Historical Figures</p>
            <h1
      ...
      <section className="bg-[#cc3322] border-b-4 border-[#14100a] py-4 px-8 md:px-16 flex flex-wrap gap-8 items-center">
        {[
          ...
          <div key={s.label} className="flex items-baseline gap-2">
            <span className="font-display text-[2.2rem] text-white leading-none">{s.n}</span>
            <span className="font-ui text-[11px] uppercase tracking-[0.1em] text-white/60">{s.label}</span>
          </div>
        ))}
        <div className="ml-auto hidden md:flex items-center gap-6">
          {activeGroups.map((g) => (
            <a key={g.label} href={`#${g.label.replace(/\s+/g, "-").toLowerCase()}`} className="no-underline font-ui text-[10px] uppercase tracking-[0.12em] text-white/50 hover:text-white transition-colors whitespace-nowrap">
            ...
            {featured.map((person, i) => (
              ...
              >
                ...
                <div className="relative z-10 p-5 w-full">
                  ...
                  <p className="font-display text-[#f2e6c8] text-[clamp(16px,2vw,24px)] leading-tight group-hover:text-[#4A6A9B] transition-colors">
                    {person.name}
                  </p>
                  {(person.birthYear || person.deathYear) && (
                    <p className="font-ui text-[11px] text-[#f2e6c8]/35 mt-1">
          ...
          return (
            <section key={group.label} id={groupId}>
              ...
              <div className="border-t-[3px] border-[#14100a] pt-6 mb-8 flex items-end justify-between gap-4">
                <div>
                  <p className="font-ui text-[11px] font-semibold tracking-[0.28em] uppercase flex items-center gap-2 before:content-[''] before:w-4 before:h-[2px] before:block"
              ...
              {/* Card grid */}
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[#1a3a72]/12">
                {members.map((person) => (
                  <NextLink
                    key={person.id}
                    href={person.slug ? `/people/${person.slug}` : `/people/${person.id}`}
                  ...
                  >
                    {/* Portrait column */}
                    <div className="w-[72px] flex-shrink-0 overflow-hidden bg-[#1a3a72]/6 group-hover:bg-[#1a3a72]/30 transition-colors relative">
                      {person.imageUrl ? (
                        <Image
                          src={person.imageUrl}
                          alt={person.name}
                    ...
                    <div className="flex-1 min-w-0 p-4">
                      ...
                      {(person.birthYear || person.deathYear) && (
                        <p className="font-ui text-[11px] text-[#14100a]/35 group-hover:text-[#f2e6c8]/35 transition-colors mt-0.5 tabular-nums">
                          {person.birthYear ?? "?"}{person.deathYear ? `–${person.deathYear}` : ""}
                        </p>
                      )}
                      <p
                        className="font-ui text-[10px] uppercase tracking-[0.1em] mt-2 transition-colors group-hover:text-[#4A6A9B]"
                        style={{ color: group.color }}
                      >
                        {person.roles.slice(0, 2).join(" · ")}
                      </p>
                      {person.primaryTown && (
                        <p className="font-ui text-[11px] text-[#14100a]/30 group-hover:text-[#f2e6c8]/30 transition-colors mt-1.5">
