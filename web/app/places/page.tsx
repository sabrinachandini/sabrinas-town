    ...
    <div className="bg-cream min-h-screen">
      ...
      <section className="bg-[#1a3a72] border-b-4 border-[#14100a] py-20 px-8 md:px-16 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto">
          ...
          <p className="font-ui text-[9px] uppercase tracking-[0.28em] text-[#e8b84b] mb-4">Historic Sites</p>
      ...
      <div className="border-b border-[#14100a]/10 bg-cream sticky top-0 z-10 overflow-x-auto">
        <div className="max-w-[1200px] mx-auto px-8 md:px-16 flex gap-6 py-3">
          {TYPE_ORDER.filter((t) => (grouped[t]?.length ?? 0) > 0).map((type) => (
            <a key={type} href={`#${type}`} className="no-underline font-ui text-[9px] uppercase tracking-[0.12em] text-[#14100a]/40 hover:text-[#cc3322] transition-colors whitespace-nowrap">
          ...
          return (
            <section key={type} id={type}>
              <div className="border-t-[3px] border-[#14100a] pt-6 mb-8 flex items-baseline justify-between">
                ...
                <span className="font-ui text-[9px] uppercase tracking-[0.1em] text-[#14100a]/30">{typePlaces.length}</span>
              </div>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {place.featured && (
                          <span className="font-ui text-[7px] uppercase tracking-[0.1em] text-[#e8b84b] border border-[#e8b84b]/40 px-1.5 py-0.5">Featured</span>
                        )}
                      </div>
                      <p className="font-ui text-[9px] uppercase tracking-[0.1em] text-[#1a3a72]/60 mt-1">
                    ...
                    <div className="flex-shrink-0 hidden md:flex flex-col items-end gap-1">
                      {place.hours && (
                        <span className="font-ui text-[9px] text-[#14100a]/30">{place.hours}</span>
                      )}
                      {place.admission && (
                        <span className="font-ui text-[9px] uppercase tracking-[0.08em] text-[#14100a]/40 border border-[#14100a]/10 px-2 py-0.5">{place.admission}</span>
                      )}
                    </div>
                  </NextLink>
                ))}
